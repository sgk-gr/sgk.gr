...
const CITIES = [
  "Αθήνα", "Θεσσαλονίκη", "Πάτρα", "Λάρισα", "Ηράκλειο", "Ιωάννινα", "Βόλος", 
  "Χανιά", "Ρόδος", "Χαλκίδα", "Τρίκαλα", "Κατερίνη", "Σέρρες", "Αλεξανδρούπολη", 
  "Ξάνθη", "Καβάλα", "Κομοτηνή", "Κέρκυρα", "Δράμα", "Κοζάνη", "Καρδίτσα", 
  "Κόρινθος", "Πτολεμαΐδα", "Ρέθυμνο", "Σπάρτη", "Μυτιλήνη", "Χίος", "Σάμος", 
  "Κως", "Καλαμάτα", "Άργος", "Ναύπλιο", "Θήβα", "Λιβαδειά", "Λαμία", "Καστοριά", 
  "Φλώρινα", "Γρεβενά", "Έδεσσα", "Νάουσα", "Βέροια", "Πρέβεζα", "Άρτα"
];
...
async function runAll() {
  console.log(`\n==================================================`);
  console.log(`Starting Greek Dentists (Οδοντίατροι) Big Scraper`);
  console.log(`==================================================`);
  
  const allProspectsMap = new Map();
  const industry = "Οδοντιατρείο";
  
  const searchTerms = ["οδοντιατρείο", "οδοντίατρος", "dentist", "dental clinic"];
  const domains = ["gmail.com", "yahoo.gr", "hotmail.com"];
  
  // We will run 40 queries targeting different cities and terms
  ...
  for (let i = 0; i < totalQueries; i++) {
    ...
    // Mix standard city searches and "facebook/instagram" keywords searches
    let query;
    if (i % 5 === 0) {
      query = `"${domain}" facebook ${term} ${city}`;
    } else {
      query = `"${domain}" ${term} ${city}`;
    }
    
    console.log(`\n[Query ${i + 1}/${totalQueries}] Searching: ${query}...`);
    try {
      ...
      for (const res of results) {
        ...
        for (const email of emails) {
          ...
          const isFree = FREE_EMAIL_DOMAINS.includes(domainName);
          if (!isFree) continue;
          ...
          const finalName = rawName.length > 2 ? rawName : `Οδοντιατρείο - ${city}`;
      ...
      console.log(`Added ${addedThisQuery} unique prospects. Total so far: ${allProspectsMap.size}`);
    } catch (err) {
  ...
  console.log(`\n==================================================`);
  console.log(`Big Scrape completed. Total unique leads collected: ${finalLeads.length}`);
  console.log(`==================================================`);
