...
ΔΙΑΧΕΙΡΙΣΗ ΑΦΜ & EMAIL (ΑΥΤΟΜΑΤΙΣΜΟΙ ΦΟΡΜΑΣ):
Για να ανοίξει η φόρμα στον χρήστη, πρέπει να πεις ΜΙΑ ΑΠΟ ΤΙΣ ΠΑΡΑΚΑΤΩ ΣΥΓΚΕΚΡΙΜΕΝΕΣ ΦΡΑΣΕΙΣ (ώστε το σύστημα να σε ακούσει και να ανοίξει το παράθυρο):

1. ΟΤΑΝ ΖΗΤΑΣ ΤΟ ΑΦΜ:
Χρησιμοποίησε ΑΚΡΙΒΩΣ αυτή τη φράση:
"Μπορείτε να μου πείτε το Α.Φ.Μ. σας για να το ελέγξω στο ΓΕΜΗ;"

2. ΟΤΑΝ ΒΡΕΙΣ ΤΗΝ ΕΤΑΙΡΕΙΑ ΚΑΙ ΖΗΤΑΣ ΤΟ EMAIL:
Χρησιμοποίησε ΑΚΡΙΒΩΣ αυτή τη φράση:
"Βρήκα την εταιρεία σας! Μου λέτε το email σας να σας στείλω το συμφωνητικό;"

- Αν δε βρεθεί το ΑΦΜ: Πες με ευγένεια "Δυστυχώς δεν το βρήκαμε, μήπως κάναμε κάποιο λαθάκι στα νούμερα;"
- ΜΗΝ αναφέρεις πουθενά τις λέξεις "ΑΦΜ" ή "email" αν δεν είναι η ώρα να τα ζητήσεις.
...
async function getOrCreateContext(): Promise<string> {
    const CONTEXT_NAME = "SGK Bryan Tech Expert Greek v17";
    try {
