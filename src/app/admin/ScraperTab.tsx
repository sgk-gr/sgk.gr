...
import { 
  Search, Sparkles, Mail, Trash2, Loader2, RefreshCcw, 
...
export function ScraperTab() {
  ...
  // Search Form State
  const [city, setCity] = useState("Καστοριά");
  const [industry, setIndustry] = useState("ξενοδοχεία");
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    const statusMessages = [
      "Σάρωση αποτελεσμάτων αναζήτησης...",
      "Εντοπισμός email επικοινωνίας...",
      "Φιλτράρισμα επιχειρήσεων χωρίς website...",
      ...
      <div className="p-6 rounded-2xl border border-white/10 bg-black/40 backdrop-blur-md">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-xs text-zinc-400 bg-white/5 px-2.5 py-1 rounded-full border border-white/5">
            Αποτελέσματα μόνο με Email & Χωρίς Website
          </span>
        </div>
        ...
        <form onSubmit={handleSearch} className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="space-y-1">
            ...
            <input 
              ...
              placeholder="π.χ. ξενοδοχεία, έπιπλα, ταβέρνες"
          ...
          <div className="space-y-1">
            ...
            <input 
              ...
              placeholder="π.χ. Καστοριά, Κοζάνη, Φλώρινα"
              className="w-full h-10 px-3 rounded-lg bg-zinc-900 border border-white/10 text-white text-sm focus:outline-none focus:border-orange-500 transition-colors"
              disabled={searching}
            />
          </div>
          <div className="flex items-end">
            <button 
              type="submit"
              disabled={searching}
              className="w-full h-10 bg-orange-500 hover:bg-orange-600 disabled:bg-orange-800 text-black font-semibold rounded-lg flex items-center justify-center gap-2 transition-colors text-sm"
            >
              {searching ? (
                <>
